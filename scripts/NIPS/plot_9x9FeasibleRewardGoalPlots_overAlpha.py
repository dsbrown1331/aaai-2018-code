...
filePath = "/home/dsbrown/Code/PeARL_BIRL/data/experiment5_1/"
    ...
    for bound_type in bounds:
        ...
        for numDemo in numDemos:  
            ...
            for rep in range(num_reps):
                if(step <= 0.005):
                    filename = filePath + "numdemos" +  str(numDemo) + "_alpha" + str(alpha) + "_chain" + str(chain_length) +  "_step" + str(step) + "000_L1sampleflag" + str(sample_flag) +  "_rolloutLength" + str(rolloutLength) + "_rep" + str(rep)+ ".txt"
                #print "rep:",rep
                elif(step < 0.1):
        ...
        plt.plot(numDemos, average_bound_error, fmts[alphas.index(alpha)], label=bound_type+", c="+str(alpha), lw=2)
        ...
        #plt.savefig("upperBoundError9x9gridgoal_exp5_1_alpha" + str(alpha) + ".png")
        ...
        plt.plot(numDemos, accuracies, fmts[alphas.index(alpha)], label= bound_type+", c="+str(alpha), lw = 2)
        ...
        plt.legend(loc='best',fontsize=19)
        plt.tight_layout()
        #plt.savefig("accuracy9x9gridgoal_exp5_1_overAlpha_" + str(alpha) + ".png") 
...
for alpha in [1,10,100]:
    ...
    plt.savefig("upperBoundError9x9gridgoal_exp5_1_overAlpha_VaR99.png")
    ...
    #plt.plot([numDemos[0], numDemos[-1]],[0.95, 0.95], 'k--', lw=1)
    plt.xticks(numDemos, fontsize=18) 
    plt.yticks([0.95, 0.96, 0.97, 0.98, 0.99, 1.0, 1.001],[0.95, 0.96, 0.97, 0.98, 0.99, 1.0,''], fontsize=18)
#    plt.axis([1,8,0.97, 1.01])
    #plt.yticks(fontsize=18)
    plt.legend(loc='best',fontsize=19)
    plt.tight_layout()
    plt.savefig("accuracy9x9gridgoal_exp5_1_overAlpha_VaR99.png") 

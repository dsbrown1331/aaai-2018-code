...
#plot results for experiment3_1
#number of rollouts is changed
...
filePath = "/home/daniel/Code/PeARL_BIRL/data/experiment3_1/"
...
for bound_type in bounds:
    ...
    for numDemo in numDemos: 
        for rolloutLength in rolloutLengths: 
            true_perf_ratio = []
            predicted = []
            bound_error = []  
            ...
            for rep in range(num_reps):
                #print "rep:",rep
                if(step < 0.1):
                    filename = filePath + "numdemos" +  str(numDemo) + "_alpha" + str(alpha) + "_chain" + str(chain_length) +  "_step" + str(step) + "0000_L1sampleflag" + str(sample_flag) + "_rolloutLength" + str(rolloutLength) + "_rep" + str(rep)+ ".txt"
                else:
                    filename = filePath + "numdemos" +  str(numDemo) + "_alpha" + str(alpha) + "_chain" + str(chain_length) +  "_step" + str(step) + "00000_L1sampleflag" + str(sample_flag) + "_rolloutLength" + str(rolloutLength) + "_rep" + str(rep)+ ".txt"
                #print filename
                f = open(filename,'r')   
        ...
        plt.xlabel("length of demonstrations", fontsize=19)
        ...
        plt.plot(rolloutLengths, average_bound_error, fmts[bounds.index(bound_type)], label=bound_type, lw=2)
        #plot dotted line across diagonal
        #plt.plot([0,true_perf_ratio[-1]],[0,true_perf_ratio[-1]], 'k:')
        plt.xticks(rolloutLengths,fontsize=18) 
        plt.yticks(fontsize=18) 
        plt.legend(loc='best', fontsize=19)
        plt.tight_layout()

        plt.savefig("rollout_upperBoundError9x9gridgoal_exp3_1.png")

        fig_cnt = 2
        plt.figure(fig_cnt)
        #plt.title(r"5x5 navigation domain, $\alpha = $" + str(alpha) + ", $step = $" + str(step))
        plt.xlabel("length of demonstration", fontsize=19)
        plt.ylabel("accuracy", fontsize=19)
        plt.plot(rolloutLengths, accuracies, fmts[bounds.index(bound_type)], label= bound_type, lw = 2)
        #plot 95% confidence line
        plt.plot([rolloutLengths[0], rolloutLengths[-1]],[0.95, 0.95], 'k--', lw=1)
        plt.axis([1, 8, 0.8, 1.003])
        plt.xticks(rolloutLengths, fontsize=18) 
        #plt.yticks([0.9, 0.925, 0.95, 0.975, 1.0, 1.001],[0.9, 0.925, 0.95, 0.975, 1.0,''], fontsize=18)
        plt.yticks(fontsize = 18)
        plt.legend(loc='best',fontsize=19)
        plt.tight_layout()
        plt.savefig("rollout_accuracy9x9gridgoal_exp3_1.png") 

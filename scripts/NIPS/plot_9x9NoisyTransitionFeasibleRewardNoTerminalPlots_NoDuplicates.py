...
filePath = "/u/dsbrown/Code/PeARL_BIRL/data/experiment10_1/"
...
for bound_type in bounds:
    ...
    for numDemo in numDemos:  
        ...
        for rep in range(num_reps):
            if(step <= 0.005):
                filename = filePath + "numdemos" +  str(numDemo) + "_alpha" + str(alpha) + "_chain" + str(chain_length) +  "_step" + str(step) + "000_L1sampleflag" + str(sample_flag) +  "_rolloutLength" + str(rolloutLength) +  "_stochastic" + str(stochastic) + "_rep" + str(rep)+ ".txt"
            #print "rep:",rep
            elif(step < 0.1):
                filename = filePath + "numdemos" +  str(numDemo) + "_alpha" + str(alpha) + "_chain" + str(chain_length) +  "_step" + str(step) + "0000_L1sampleflag" + str(sample_flag) +  "_rolloutLength" + str(rolloutLength) + "_stochastic" + str(stochastic) + "_rep" + str(rep)+ ".txt"
#            else:
#                filename = filePath + "numdemos" +  str(numDemo) + "_alpha" + str(alpha) + "_chain" + str(chain_length) +  "_step" + str(step) + "00000_L1sampleflag" + str(sample_flag) + "_rep" + str(rep)+ ".txt"
        ...
        print "bound ave error", np.mean(bound_error) 
    fig_cnt = 1
    plt.figure(fig_cnt)
    ...
    plt.savefig("upperBoundError9x9NoisyTgridNoTerminal_NoDups_alpha" + str(alpha) + ".png")
    ...
    ax2.plot(numDemos, accuracies, fmts[bounds.index(bound_type)], label= bound_type, lw = 2)
    #plot 95% confidence line
    #plt.plot([numDemos[0], numDemos[-1]],[0.95, 0.95], 'k--', lw=1)
    plt.xticks(numDemos, fontsize=18) 
    plt.yticks([0.88, 0.90, 0.92, 0.94, 0.96, 0.98, 1.0, 1.001],[0.88, 0.90, 0.92, 0.94, 0.96, 0.98, 1.0,''], fontsize=18)
    #plt.yticks(fontsize=18)

...
filePath = "/u/dsbrown/Code/PeARL_BIRL/data/experiment9_1/"
...
for bound_type in bounds:
    ...
    for numError in numErrors:  
        ...
        for rep in range(num_reps):
            filename = filePath + "numdemos" +  str(numDemo) + "_alpha" + str(alpha) + "_chain" + str(chain_length) +  "_step" + str(step) + "0000_L1sampleflag" + str(sample_flag) +  "_numErrors" + str(numError) + "_stochastic" + str(stochastic) + "_rep" + str(rep)+ ".txt"
#            else:
#                filename = filePath + "numdemos" +  str(numDemo) + "_alpha" + str(alpha) + "_chain" + str(chain_length) +  "_step" + str(step) + "00000_L1sampleflag" + str(sample_flag) + "_rep" + str(rep)+ ".txt"
    ...
    plt.xlabel("number of errors in evaluation policy", fontsize=19)
    ...
    plt.plot(numErrors, average_bound_error, fmts[bounds.index(bound_type)], label=bound_type, lw=2)
    #plot dotted line across diagonal
    #plt.plot([0,true_perf_ratio[-1]],[0,true_perf_ratio[-1]], 'k:')
    plt.xticks(numErrors,fontsize=18) 
    plt.yticks(fontsize=18) 
    plt.legend(loc='best', fontsize=19)
    plt.tight_layout()

    plt.savefig("upperBoundError9x9NoisyT9x9NoisyT_overEvalPolicy_" + str(numDemo) + "demos.png")
    ...
    plt.plot(numErrors, accuracies, fmts[bounds.index(bound_type)], label= bound_type, lw = 2)
    ...
    plt.tight_layout()
    plt.savefig("accuracy9x9NoisyT9x9NoisyT_overEvalPolicy_" + str(numDemo) + "demos.png") 

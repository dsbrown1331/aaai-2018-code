...
filePath = "/home/dsbrown/Code/PeARL_BIRL/data/carExperiment3_1/"

evalPolicyName = "stay_on_road"
print evalPolicyName
...
for bound_type in bounds:
    ...
    for numDemo in numDemos:  
        ...
        for rep in range(num_reps):
            filename = filePath + "simple_" + evalPolicyName + "_alpha" + str(alpha) + "_chain" + str(chain_len) + "_step" + str(mcmc_step)+ "0000_L1sampleflag" + str(sample_flag) + "_demoLength" + str(demo_length)+ "_mcRollout" + str(mc_numRollouts) + "_rep" + str(rep)+ ".txt";
            #print filename
            f = open(filename,'r')   
        ...
        print "ave worst cast", np.mean(worst_cases)
        print "ave predicted", np.mean(predicted)
        print "ave actual", np.mean(actuals)
        accuracy = 0.0
        ...
        print "ave bound error", np.mean(bound_error)
        print "stdev bound error", np.std(bound_error)
        print 
    fig_cnt = 1
    plt.figure(fig_cnt)

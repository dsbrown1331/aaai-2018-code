...
vector<vector<bool> > generateAllSubsets(int size)
{
    cout << size << endl;
    ...
    //vector<bool> toAdd = indices;
    //subsets.push_back(toAdd);
    //don't add all zeros
    for(int i=1; i<int(pow(2,size)); i++) 
    {
...
double* updown_l1_norm_walk(const double* weights, int dim, double step)
{
    ...
    //cout << "rand_dim1: " << rand_dim1 << ", rand_dim2: " << rand_dim2 << endl; 
    int rand_dir1 = 0;;
    int rand_dir2 = 0;
    if(isEqual(weights[rand_dim1], 0.0))
    {
        //cout << "first dim 0" << endl;
    ...
    //cout << "rand_dir1: " << rand_dir1 << ", rand_dir2: " << rand_dir2 << endl; 
...
pair<double,double> manifold_l1_step(double cur_val1, double cur_val2, string rand_dir, double rand_step)
{
    ...
    //cout << "val 1: " << cur_val1 << endl;
    //cout << "val 2: " << cur_val2 << endl;
    ...
    //cout << "cycle indx: " << cycle_indx << endl;
    int cycle_dir;
    ...
    {    
        ...
        //cout << "new vals" << cur_val1 << "," << cur_val2 << endl;
        //cout << "new cycle index: " << cycle_indx << endl;
    }    
    //go back one step to undo last cycle turn
    //cycle_indx = (cycle_indx - 1) % 4;
    //cout << "final cycle index: " << cycle_indx << endl;
    //multiply by signs to get in correct quadrant
    if(rand_dir == "clockwise")
        cycle_pos = clockwise_pos[cycle_indx];
    else
        cycle_pos = counterclockwise_pos[cycle_indx];
    //cout << "final cycle pos: " << cycle_pos << endl;
    if(cycle_pos == "-+")
        cur_val1 *= -1;
    else if(cycle_pos == "+-")
        cur_val2*= -1;
    ...
    //print np.sum(np.abs(cur_val))
    assert(isEqual(abs(cur_val1) + abs(cur_val2), slack));
    //print "new val", cur_val
...
double* random_manifold_l1_step(const double* weights, int dim, double step_size, int num_steps)
{
    ...
    for(int s = 0; s < num_steps; s++)
    {
        ...
        do
        {
            ...
            while(rand_dim2 == rand_dim1)
                rand_dim2 = rand() % dim;
            //cout << "picking dims" << endl;
        }
        while(isEqual(sample[rand_dim1], 0.0) && isEqual(sample[rand_dim2], 0.0));
        //cout << "stepping along dim: " << rand_dim1 << " and dim: " << rand_dim2 << endl;
...
double* take_all_manifold_l1_steps(const double* weights, int dim, double step_size)
{
    ...
    for(pair<int,int> t : tuples)
    {
        ...
        int dim1 = t.first;
        int dim2 = t.second;
        //cout << "flipping on " << dim1 << ", " << dim2 << endl;

...
int main() 
{
    ...
    const vector<unsigned int> numDemos = {9};            //number of demos to give
    const vector<unsigned int> numErrors = {0,2,4,8,16,32,64};
    ...
    const vector<double> alphas = {100,10}; //50                    //confidence param for BIRL
...
for(unsigned int numError : numErrors)
{
    //iterate over alphas
        ...
        for(double step : steps)
        {
            for(unsigned int rep = 0; rep < reps; rep++)
            {
                ...
                string filename = "numdemos" +  to_string(numDemo) 
                                ...
                                + "_numErrors" + to_string(numError)
                                + "_stochastic" + to_string(stochastic)
                                + "_rep" + to_string(rep)+ ".txt";
                cout << filename << endl; 
                ofstream outfile("data/experiment9_1/" + filename);
                ...
                //make the EVAL Policy a copy of the opt_policy
                vector<unsigned int> eval_pi (mapMDP->getNumStates());
                for(unsigned int i=0; i<opt_policy.size(); i++)
                    eval_pi[i] = opt_policy[i];
...
//                mapMDP->displayPolicy(eval_pi);
                ...
                //Now change numError of the states to be using a non-optimal action
                ...
                for(int i=0; i<numError; i++)
                {
                    ...
                    while(find(changeStates.begin(), changeStates.end(), s) != changeStates.end());
                    //save state to be changed so i don't repeat
                    changeStates.push_back(s);
                    //change state-action in eval_pi to non-optimal action
                    int action;
                    do{
                        action = rand() % fmdp.getNumActions();
                    }
                    while(fmdp.isOptimalAction(s, action));
                    //update state to non-optimal action
                    eval_pi[s] = action;
                
                }
                cout << "changed..." << endl;
                for(unsigned int s : changeStates)
                    cout << s << ", ";
                cout << endl;
                cout << "opt policy changed by " << numError << " states" << endl;
                mapMDP->displayPolicy(eval_pi); 

...
int main()
{
    ...
    int numActions = 3;
    double gamma = 0.9;
    int numDemoPolicyQsteps = 5000;
    int numBIRL_QSteps = 5000;     
    int demo_length = 100;
    double min_reward = -2;
    double max_reward = 2;
...
//                               -0.5}; //offroad right
    ...
    if(givePolicyDemo)
    {
        ...
        set<string> states = qbot.getQTableStates();
        for(string state : states)
        {
            unsigned int a = qbot.getArgmaxQvalues(state);
    ...
    {
        world.setVisuals(true); //also visualize the demonstration

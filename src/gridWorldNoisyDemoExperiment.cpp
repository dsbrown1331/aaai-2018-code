...
//Evaluate sensitivity to noisy demonstrations for performance bounds

//Code to run experiment shown in Figure 3 in "Efficient Probabilistic Performance Bounds for Inverse Reinforcement Learning" from AAAI 2018.
...
int main() 
{
    ...
    const vector<double> alphas = {0,1,5,10,50}; //50                    //confidence param for BIRL
...
//    double** stateFeatures = random9x9GridNavGoalWorld8Features();
    ...
    string filePath = "./data/gridworld_noisydemo_exp/";
    ...
    for(double alpha : alphas)
    {
        ...
        for(double step : steps)
        {
            for(unsigned int rep = 0; rep < reps; rep++)
            {
                ...
                string filename = "NoisyDemo_numdemos" +  to_string(numDemo) 
                                ...
                                + "_randActionProb" + to_string(rand_action_prob)
                ...
                for(unsigned int d = 0; d < numDemo; d++)
                {
                   ...
                   vector<pair<unsigned int, unsigned int>> traj = fmdp.epsilon_random_rollout(s0, rolloutLength, rand_action_prob);

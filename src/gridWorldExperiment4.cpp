...
/// experiment5_1 on lab machine stochastic = false
// experiment5_2 on lap top has noisy transitions stochatic=true #FAILED!
//experiment5_3 on lap top has noisy trans and does feasibility check on all demos, need to uncomment code and change numDemo to maxDemos in loop, TODO seems to have memory leak, all experiments got killed off...


//experiment5_1 on cs machine has no duplicates
//experiment5_2 on cs machine has duplicates
...
int main() 
{
    ...
    const vector<unsigned int> numDemos = {8};            //number of demos to give
    const int maxDemos = 8;
    const vector<unsigned int> rolloutLengths = {25};          //max length of each demo
...
for(unsigned int rolloutLength : rolloutLengths)
{
    ...
    for(double alpha : alphas)
    {
        ...
        for(double step : steps)
        {
            for(unsigned int rep = 0; rep < reps; rep++)
            {
                ...
                cout << filename << endl; 
                ofstream outfile("data/experiment5_1/" + filename);
                ...
                do{
                    ...
                    //set weights for all features except goal feature to be positive
    ...
    //                featureWeights[4] = -abs(negFeatureWeights[3])/2;
                    ...
                    for(unsigned int d = 0; d < numDemo; d++)
                    {
                       ...
                       vector<pair<unsigned int, unsigned int>> traj = mdp.monte_carlo_argmax_rollout(s0, rolloutLength);
...
/////uncomment for correct way to genearte trajectoris, I think...
//                 ///  generate numDemo demos from the initial state distribution
//                trajectories.clear(); //used for feature counts
//                for(unsigned int d = 0; d < numDemo; d++)
//                {
